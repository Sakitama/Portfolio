;+function($) {
	...
	var myDate = function(elments, options) {
		...
		this.options = $.extend(true, obj, options || {});//深度拷贝
	...
	myDate.prototype = {
		...
		prevMonth: function() {
			...
			this.elments.find('.prevMonth').on('click', function() {
				var prevYear = parseInt(_this.elments.find('.year').html(), 10) - 1;
				var nowMonth = parseInt(_this.elments.find('.month').html(), 10) - 1;
				if (nowMonth == 0) {
				...
				var nowYear = parseInt(_this.elments.find('.year').html(), 10);
		...
		dateMain: function(year, month) {
			...
			if (thisDay == 0) { //如果当月一号是星期天
				prev = prevArr.slice(-7);
			} else {
				prev = prevArr.slice(-thisDay);
			}

			var num = 42 - thisDate - prev.length; //总共42个格子，减去当月天数和上个月最后的那几天的天数
			for (var i = 1; i <= num; i++) { //根据剩下的格子数目填充下个月的天数
				nextArr.push(i);
			}
